(() => {
  ...
  $('#agent-name').addEventListener('input', (event) => event.target.setCustomValidity(''));
  ...
  websiteForm.addEventListener('submit', async (event) => {
    ...
    if (/[{}<>\r\n]/u.test(requestedName)) {
      nameInput.setCustomValidity('Enter a name without markup or template characters.');
      nameInput.reportValidity();
      return;
    }
    nameInput.setCustomValidity('');
    chosenAgentName = requestedName.slice(0, 40);
